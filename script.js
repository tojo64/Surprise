...
function createParticles() {
  ...
  const particleCount = isCompactDevice ? 12 : 42;
...
async function runIntro() {
  createStars(qs(".intro-stars"), isCompactDevice ? 28 : 90);
...
function setupConstellation() {
  ...
  createStars(field, isCompactDevice ? 22 : 32, true);
...
function setupGame() {
  ...
  function spawnFallingStar() {
    ...
    let y = -30;
    const speed = (isCompactDevice ? 2.8 : 2.2) + Math.random() * (isCompactDevice ? 1.6 : 2.2);
  ...
  function startGame() {
    ...
    qsa(".falling-star", board).forEach((star) => star.remove());
    clearInterval(spawnTimer);
    spawnTimer = setInterval(spawnFallingStar, isCompactDevice ? 820 : 650);
    board.focus();
  }
...
function setupDeviceMode() {
  ...
  let savedMode = isCompactDevice ? "mobile" : "auto";
  try {
    savedMode = localStorage.getItem(storageKey) || savedMode;
  } catch (error) {
    savedMode = isCompactDevice ? "mobile" : "auto";
  }
  applyMode(savedMode);
}
...
document.addEventListener("DOMContentLoaded", () => {
  document.body.classList.toggle("compact-performance", isCompactDevice);
  createStars(qs(".star-layer"), isCompactDevice ? 55 : 180);
  createStars(qs("#heroStars"), isCompactDevice ? 38 : 110);
  createParticles();
  setupReveals();
  setupConstellation();
  setupGame();
  ...
  if (prefersReducedMotion) {
    ...
    qs("#heroTypewriter").textContent = "Hey Sara... Something special is being built for you.";
  } else {
    runIntro();
